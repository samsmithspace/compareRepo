...
def run_comparison():
    ...
    # Load custom Food-101 dataset from prepared directories
    print("Loading custom Food-101 dataset...")
    train_dataset = datasets.ImageFolder(
        root='datasets/food101_split/train', transform=transform_train
    )
    val_dataset = datasets.ImageFolder(
        root='datasets/food101_split/val', transform=transform_val
    )
    test_dataset = datasets.ImageFolder(
        root='datasets/food101_split/test', transform=transform_val
    )
...
def main():
    ...
    # Create model - use the right number of classes based on your dataset
    print("Creating model...")
    num_classes = len(train_dataset.classes)
    print(f"Number of food classes detected: {num_classes}")
